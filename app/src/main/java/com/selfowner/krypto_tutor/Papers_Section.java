...
public class Papers_Section extends AppCompatActivity {
    TextView back;
    final static int PICK_PDF_CODE = 2342;
    Button UploadFile;
    EditText nameofPaper;
    ...
    //Others
    private String UID;
    private String department,tName,tContact;
    ...
    private int flag=0;
    //default string
    private String page="PAPERS_SECTION";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_papers__section);
        firebaseAuth=FirebaseAuth.getInstance();
        FirebaseUser user=firebaseAuth.getCurrentUser();
        UID=user.getUid();
        back=findViewById(R.id.backBtn);
        UploadFile=findViewById(R.id.upload);
        nameofPaper=findViewById(R.id.nameofNotes);
        selectSemester=findViewById(R.id.selLevel);
        listView=findViewById(R.id.listview);
        DATE();
        LOADMYINFO();
        adapter=new ArrayAdapter<String>(Papers_Section.this,android.R.layout.simple_list_item_1,namelist);
        listView.setAdapter(adapter);
        back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(Papers_Section.this,Teacher_Dashboard.class));
                finish();
            }
        });
        ...
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.hasChild(UID)){
                    ...
                    tName=dataSnapshot.child(UID).child("name").getValue().toString();
                    tContact=dataSnapshot.child(UID).child("contact").getValue().toString();
                }
                Toast.makeText(Papers_Section.this, "Department Is:"+department, Toast.LENGTH_SHORT).show();
            }
            @Override
    ...
    private void uploadFile(Uri data) {
        filename=nameofPaper.getText().toString();
        levelname=selectSemester.getSelectedItem().toString();
        if(!TextUtils.isEmpty(filename) && !TextUtils.isEmpty(levelname)) {
            final ProgressDialog progressDialog=new ProgressDialog(Papers_Section.this);
            ...
            sRef.putFile(data)
                    .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                        ...
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            ...
                            databaseReference=FirebaseDatabase.getInstance().getReference(""+(tName+"_"+tContact));
                            ...
                            Toast.makeText(Papers_Section.this, "File Uploaded...!", Toast.LENGTH_SHORT).show();
                        }
                    })
...
//text_entry is an Layout XML file containing two text field to display in alert dialog
                        ...
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int whichButton) {
                                if(lev.equals("SELECT") || lev.equals("")){
                                    Toast.makeText(Papers_Section.this, "Provide CLass/Semester Information", Toast.LENGTH_SHORT).show();
    ...
    private void LISTVIEW(String Level){
        ...
        databaseReference= FirebaseDatabase.getInstance().getReference(""+(tName+"_"+tContact)).child(""+page).child(""+Level).child(""+department);
